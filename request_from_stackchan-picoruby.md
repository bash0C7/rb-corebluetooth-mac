...
- [x] **P0: `path:` source 経由で gem を載せたとき、prebuilt `lib/corebluetooth_mac/corebluetooth_mac.bundle` が consumer 側 Ruby version と不一致だと `LoadError: linked to incompatible /Users/.../libruby.X.Y.dylib` で即死する** → gem PR #1 (69c31da + 84a05bc) で対応
  ...
  - 着地: gem 側で `.ruby-version` を削除し (ライブラリは consumer の Ruby に合わせるべき)、README "When loaded from a `path:` (or git:) source" 節で「prebuilt bundle は最後に `rake compile` した Ruby ABI に linked、不一致なら consumer 側で `bundle exec rake compile` し直す」を明記。load 時自動 detection は見送り (README で十分とした)

- [x] **P1: `swift_gem` runtime dependency が rubygems.org に未公開で、consumer Gemfile に追加 path: 指定が必須** → gem PR #1 (69c31da) で README に明記
  ...
  - 着地: README "When loaded from a `path:` (or git:) source" 節で「`gem 'swift_gem', path: '...'` を consumer Gemfile に追加必須」を明記。rubygems.org への publish は将来 issue

---

## API / contract レベル (Plan 進行中に追記予定)

- [x] **P1: `Characteristic#unsubscribe` が central 下の全 subscription を purge する** → gem PR #1 (69c31da) で fix
  ...
  - 着地: `CBMSubscriptionRegistry.purgeMatching(under:, characteristicUUID:)` を新設、`unsubscribeCharacteristic` が char UUID で scope して purge。`purgeAll` は `Central#close` teardown 専用で残置 (公開 API 変更なし)

- [x] **P2: Apple CoreBluetooth が GAP (0x1800) / GATT (0x1801) services を `discoverServices(nil)` から filter する仕様を README に明記してほしい** → gem PR #1 (69c31da) で README "Limitations" に追記
...
- [x] **P2: README の subscribe 例が Ractor 必須に見える** → gem PR #1 (69c31da) で "Simple polling (main thread)" を先頭に、"Ractor pump (parallel)" を後段に再構成
